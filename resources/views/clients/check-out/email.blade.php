...
<head>
    ...
    <title>Order Notification | Spider Store</title>
</head>
                ...
                <div class="container-fluid">
                    ...
                    <div class="row mt-5" style="margin-top: 35px; display: flex;">
                        <div class="col-6"
                            ...
                            <b>{{ ($order->user->first_name ?? 'A') . ', ' . ($order->user->last_name ?? "Nguyen Van")  }}</b>
                            <br>
                            {{-- <span>
                                <a style="color: white !important;" href="mailto:{{ $order->email ?? "test" }}" target="_blank">{{ $order->email ?? "test" }}</a>
                            </span> --}}
                            <br>
                            <span>{{ $order->user->phone_num ?? '0123456789' }}</span>
                        </div>
                        <div class="col-6" style="flex: 0 0 50%; width: 50%; box-sizing: border-box;">
                            ...
                            <br>
                            <b>Address:</b> {{$order->received_address }}
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            @foreach($order->orderdetail as $orderDetail)
                                <tr>
                                    <td style="border-top: 1px solid #dee2e6; padding: 5px 0;">
                                        {{ $orderDetail->product->product_name . ' (x' . $orderDetail->quantity . ')'}}
                                    </td>
